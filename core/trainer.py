...
from models import hmr, hmrs, SMPL
...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        if self.options.regressor == 'hmr':
        ...
        elif self.options.regressor == 'hmrs':
            # HMRS model
            ...
            self.smpl_female = SMPL(model_path=path_config.SMPL_MODEL_DIR,
                                    gender='female',
                                    create_transl=False).to(self.device)
            self.model = hmrs(path_config.SMPL_MEAN_PARAMS, pretrained=True)
        ...
        if cfg.MODEL.HMRS.AUX_SUPV_ON:
            self.iuv_maker = IUV_Renderer(output_size=cfg.MODEL.HMRS.DP_HEATMAP_SIZE)
    ...
    def train_step(self, input_batch):
        ...
        if self.options.regressor == 'hmrs' and cfg.MODEL.HMRS.AUX_SUPV_ON:
            ...
            iuv_image_gt = torch.zeros((batch_size, 3, cfg.MODEL.HMRS.DP_HEATMAP_SIZE, cfg.MODEL.HMRS.DP_HEATMAP_SIZE)).to(self.device)
        ...
        len_loop = len(preds_dict['smpl_out']) if self.options.regressor == 'hmrs' else 1

        for l_i in range(len_loop):

            if self.options.regressor == 'hmrs':
                if l_i == 0:
    ...
    def visualize(self, it, target, stage, preds, losses=None):
        ...
        dp_out = preds['dp_out'][-1] if cfg.MODEL.HMRS.AUX_SUPV_ON else None
